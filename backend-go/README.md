...
Browser WebRTC audio
  ...
  → Google / Gemini / Azure / GPT Realtime Translate
...
| GPT Realtime Translate | 24 kHz PCM16 | `/v1/realtime?intent=transcription`; source interim deltas and completed finals; 650 ms silence/30-second hard boundary; bounded reconnect with one-second audio replay |
...
`sequence` is monotonic for the room agent and lets clients discard a delayed lossy Draft after a newer final packet. Google interim revisions and their final share a `segmentId`; `role`, `languageCode`, and `turnId` are additive metadata used by Gemini source/translation pairs. GPT Realtime Translate publishes `role: "source"` with the configured `OPENAI_LANGUAGE_CODE`; it does not emit translation packets. Spacing is normalized once at the agent output boundary; only Google Thai removes spaces between adjacent Thai characters.

Gemini `turnId` values are application-level pseudo-turns rather than deterministic model turns. The provider observes the unchanged PCM stream and requests a turn boundary after 650 ms of low-energy audio. Source text becomes final at that boundary; delayed translation keeps the same `turnId` during a fixed 500 ms grace period. This keeps delayed translated output with the preceding source in typical pauses without delaying the source final, but alignment remains best-effort rather than sentence-perfect.
